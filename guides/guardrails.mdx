...
| 6. Compliance gate (AC-6) | Pre-execution PII/PHI scan — data never moves if a pattern matches | `GDPR_DATA_MINIMISATION_VIOLATION`, `HIPAA_MINIMUM_NECESSARY_VIOLATION` |
...
Borderline requests — flagged by the pre-filter as `REVIEW` — are routed to the review queue before execution. A human reviewer approves or denies via the API. If no action is taken within the TTL window, the request is auto-denied (fail-closed).

```
Pre-filter: REVIEW
    |
    v
Review queue (Redis-backed, TTL: 15 min)
    |
    +-- approved -> tool executes
    +-- denied   -> DENY, reviewed_by in forensic envelope
